...
class Device:
    ...
    def ipv4_to_int(ip_string):
        ...
        Purpose:
            Layer 3 routing needs to compare an address against subnet ranges.
            Doing this manually (instead of relying on an external/standard
            address library) keeps the project self-contained and shows the
            underlying networking logic the assignment is meant to demonstrate.
        """
        ...
        for octet in octets:
            ...
            if number < 0 or number > 255:
                raise ValueError(f"Invalid IPv4 octet in address: {ip_string}")
            # Shift the running value left by one byte and add this octet.
    ...
    def prefix_to_netmask(prefix_length):
        """
        Build a 32-bit netmask integer from a CIDR prefix length.

        For example, a prefix length of 24 produces the mask 255.255.255.0.
        """
        ...
        # Set the top `prefix_length` bits and keep the result inside 32 bits.
        return (0xFFFFFFFF << (32 - prefix_length)) & 0xFFFFFFFF

    def network_contains(self, cidr, ip_string):
        """
        Return True if ip_string falls inside the CIDR subnet (e.g. 10.0.2.0/24).

        Purpose:
            Implements subnet matching by hand using bitwise masking instead of
            an address-handling library, so Layer 3 routing relies only on basic
            Python arithmetic.
        """

        network_string, _, prefix_string = cidr.partition("/")
        netmask = self.prefix_to_netmask(int(prefix_string))

        # Apply the mask to both addresses; they share a subnet if the masked
        # network portions are identical.
        network_bits = self.ipv4_to_int(network_string) & netmask
        address_bits = self.ipv4_to_int(ip_string) & netmask

        return network_bits == address_bits
    ...
    def lookup_route(self, destination_ip):
        ...
        for route in self.routing_table:
            if self.network_contains(route["destination_network"], destination_ip):
...
class Host(Device):
    ...
    def send_application_data(
        ...
        # Work out how many Layer 4 DATA segments are needed.
        # This makes segmentation visible when the message is larger than 500 bytes.
        total_segments = (data_size + MAX_SEGMENT_DATA_SIZE - 1) // MAX_SEGMENT_DATA_SIZE

        if total_segments > 1:
            self.log(
                "Layer 4",
                f"Application data segmented into {total_segments} DATA segments "
                f"(max data per segment={MAX_SEGMENT_DATA_SIZE})",
            )

        # Used only for clearer logging of multi-segment messages.
        segment_number = 1
        ...
        while remaining > 0:
            ...
            # Show which segment is currently being prepared when segmentation is required.
            if total_segments > 1:
                self.log(
                    "Layer 4",
                    f"Preparing DATA segment {segment_number}/{total_segments}. Data size={segment_data_size}",
            )
            ...
            # rdt2.2 alternating-bit protocol: flip the sequence number ONLY
            # after the current segment has been positively acknowledged, so
            # successive segments alternate 0 -> 1 -> 0 -> 1. This is visible in
            # the logs through the per-segment "(DATA, seq=...)" lines.
            self.next_send_seq = 1 - self.next_send_seq

            remaining -= segment_data_size

            # Move the segment counter forward after this DATA segment is acknowledged.
            segment_number += 1
    ...
    def _send_data_segment_until_ack(self, segment, destination_ip):
        ...
        while True:
            ...
            # In rdt2.2 a correct ACK carries the SAME sequence number as the
            # DATA segment it acknowledges. Receiving that ACK completes this
            # segment and allows the sequence number to alternate.
            if self.last_ack_received == segment.seq_num:
                return

            # Anything else means the segment must be retransmitted WITHOUT
            # changing its sequence number: either no ACK arrived, or an ACK for
            # the other sequence number arrived (a stale/duplicate ACK, which the
            # sender ignores). Under the spec's lossless assumption this branch
            # never executes, but it is required for correct rdt2.2 behaviour.
            self.log(
                "Layer 4",
    ...
    def receive_segment_from_network(self, segment, source_ip):
        ...
        if segment.is_data():
            if segment.seq_num == self.expected_receive_seq:
                ...
                # Record the ACK number, then alternate the expected sequence so
                # the next in-order DATA segment must use the opposite bit. The
                # alternation is reflected in the seq numbers of subsequent
                # "(DATA, seq=...)" and "ACK, seq=..." log lines.
                ...
                # A DATA segment with the unexpected sequence number is a
                # duplicate. It is not delivered again; instead the receiver
                # repeats its last ACK so the sender can recover.
